...
pub fn fill_content_to_comply_with_page_blob_size(content: &mut Vec<u8>, byte_to_fill: u8) {
    if content.len() == 0 {
        return;
    }

    let pages_amount = get_pages_amount_by_size(
        content.len(),
        my_azure_storage_sdk::page_blob::consts::BLOB_PAGE_SIZE,
    );

    let required_size = pages_amount * my_azure_storage_sdk::page_blob::consts::BLOB_PAGE_SIZE;
...
mod tests {
    ...
    fn test_page_blob_resize() {
        ...
        fill_content_to_comply_with_page_blob_size(&mut content, 32u8);
